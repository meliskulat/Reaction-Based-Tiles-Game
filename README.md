# Reaction-Based-Tiles-Game
Developed an interactive browser-based reaction game using JavaScript and DOM manipulation. The game features a 4x4 dynamic grid where a random tile turns black until clicked within a 10-second time limit, challenging users to react quickly. The game values less time passed between clicking the black tiles more, as the time between correct clicks increase, the point earned from clicking the black box drops. Implemented real-time scoring logic where faster clicks yield higher points, along with a high score system that updates automatically when a new record is achieved.


## Project Overview 

The key aspects of this project include:

- The game features an interactive 4x4 grid where tiles are activated randomly using JavaScript to test player actions
- A real-time scoring logic is implemented with a system that rewards faster reaction times, each tile offers maximum 10 points
-High score persistence is applied by using localStorage to save and display the best performance     
- The user experience is enhanced with smooth CSS transitions, confetti effect for new high score
- Automatic game management handles the 10-second timer and it states a prompt to play again for the user
 

## Tech Stack  

**Language**: JavaScript, CSS, HTML  
**Technologies**: DOM Manipulation    
**Environment**: Visual Studio Code  

## Getting Started  

### Prerequisites  

- Local Web Browser
- Visual Studio Code: Recommended IDE for managing and running the project files
- Live Server(Optional): VS code extension for real-time development 

### Installation  

- Install Live Server: Open VS Code and go to the Extension Marketplace 
  
- Clone the repository
  ```bash
  git clone https://github.com/meliskulat/Reaction-Based-Tiles-Game.git  
  ```
  
  
## Usage  

- Launch with Live Server: Open the project folder and `index.html` in VS Code.  
- Right click on `index.html` and choose `Open with Live Server`.  
- On the web browser game will be live.  
- Choose your player.     
- You can follow the instructions on the page.  
- Your highest score will be saved.  

## Contributors  

- [Melis Külat](https://github.com/meliskulat)  
- [İpek Kısaoğlu]()
